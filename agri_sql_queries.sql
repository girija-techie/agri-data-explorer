...
SELECT 
    `Year`, 
    `State Name`, 
    SUM(`RICE PRODUCTION (1000 tons)`) AS production,
    DENSE_RANK() OVER (PARTITION BY `Year` ORDER BY SUM(`RICE PRODUCTION (1000 tons)`) DESC) AS rank
FROM agri_data GROUP BY `Year`, `State Name`) as t where t.rank <=3 order by `Year`, rank
...
SELECT 
    a.`State Name`,
    ( 
        (SELECT `OILSEEDS PRODUCTION (1000 tons)` 
         FROM agri_data 
         WHERE `State Name` = a.`State Name` 
         AND Year = (SELECT MAX(Year) FROM agri_data)
         LIMIT 1
        )
        -
        (SELECT `OILSEEDS PRODUCTION (1000 tons)` 
         FROM agri_data 
         WHERE `State Name` = a.`State Name` 
         AND Year = (SELECT MAX(Year) - 5 FROM agri_data)
         LIMIT 1
        )
    ) AS Growth
FROM agri_data a
